...
class ReplayBuffer:
    ...
    def load_dataset(self, dataset: Dict[str, np.ndarray]) -> None:
        observations = np.array(dataset["state"], dtype=self.obs_dtype)
        next_observations = np.array(dataset["next_state"], dtype=self.obs_dtype)
        actions = np.array(dataset["action"], dtype=self.action_dtype)
        rewards = np.array(dataset["reward"], dtype=np.float32).reshape(-1, 1)
        terminals = np.array(dataset["terminal"], dtype=np.float32).reshape(-1, 1)
